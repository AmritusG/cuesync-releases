...
// =============================================================================
// CUESYNC-9 §0.12 / step-4-contingency compliance — the GtkBackend Windows
// window-present patch (round 13): its presence, placement, idempotency, read-only
// scoping, pin, disjoint hunk, and no-regression on the two prior patches, across
// all three GtkBackend-compiling CI legs (macos, windows-build, windows-test), and
// the checked-in `.patch` file itself.
//
// Root cause (findings §0.12, correcting §0.10/§0.11): SwiftCrossUI shows the
// INITIAL WindowGroup window via backend.show(window:) on isFirstUpdate
// (gtk_widget_set_visible) and NEVER backend.activate(window:) — activate/present is
// reached only via openWindow(id:)/bringWindowForward(). On Windows, show maps but
// does not RAISE the surface, so the window stays behind the foreground cmd.exe
// launcher console the probe box starts CueSync.exe from and paints no visible
// pixels (round-12 RGB pixel audit, §0.11). The patch makes GtkBackend.show also
// present() the initial window on Windows, via GTK's own gtk_window_present().
//
...
final class CUESYNC9WindowPresentFindingsTests: XCTestCase {
    ...
    /// specs/CUESYNC-9-findings.md documents the round-13 window-present root cause + fix.
    func testFindingsDocumentsRound13WindowPresentRootCauseAndFix() throws {
        let text = try String(contentsOf: RepoPathsWP.findings, encoding: .utf8)
        XCTAssertTrue(text.contains("§0.12"),
            "specs/CUESYNC-9-findings.md must contain a §0.12 section documenting the round-13 fix")
