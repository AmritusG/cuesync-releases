...
final class PortComplianceTests: XCTestCase {
    ...
    /// CUESYNC-7 lands each top-level shell file (§C added HeaderView/FooterView, §D added
    /// CollapsibleSection).
    func testUIShellFilesAreFullyWrappedEndToEndInCUESYNCCROSSUIGuard() throws {
        let uiDir = Self.sourceRoot.appendingPathComponent("UI")
        for fileName in ["CueSyncApp.swift", "ContentView.swift", "HeaderView.swift", "FooterView.swift", "CollapsibleSection.swift"] {
            let url = uiDir.appendingPathComponent(fileName)
            guard let src = try? String(contentsOf: url, encoding: .utf8) else {
                XCTFail("UI/\(fileName) does not exist (spec §B)")
                continue
    ...
    /// `HeaderView.swift`/`FooterView.swift`, §D added `CollapsibleSection.swift`) — so this
    /// test's expected set grows in step with each landed section rather than staying frozen
    /// at the step-1 snapshot. It still catches a stray/duplicate top-level file the same way
    /// the `Support/` exact-file-count test does; files under `UI/`'s own subdirectories
    /// (`State/`, `Theme/`, `Sections/`, `Controls/`) are out of scope here since
    /// `contentsOfDirectory` doesn't recurse.
