...
# CUESYNC-9 §0.12 (round 13): a fourth, distinct root-cause patch against the same
...
# hunk from the other three. Distinct in root cause/file/mechanism from the
# round-9-reverted WINDOWS_INPUT_PATCH_NAME.
...
def test_dev_script_and_every_ci_leg_apply_the_two_remaining_checked_in_patches():
    """UPDATED for round 13 (specs/CUESYNC-9-findings.md §0.12): round 13 adds the
    window-present patch (show(window:) -> gtk_window_present on Windows). Exactly
    THREE named patches now touch GtkBackend.swift — gesture/interactivity +
    GSK-renderer + window-present. The round-9-reverted windows-input patch stays
    absent; a FOURTH file (that patch reappearing, or anything else) is a
    supply-chain regression, so the expected set is asserted exactly, and the
    reverted patch's absence is checked directly by name too."""
    ...
    assert gtk_patches == expected, (
        "spec §4/§0.3/§0.12: expected exactly the three named checked-in patches "
        "touching GtkBackend.swift (" + repr(expected) + ") now that round 13 added "
        "the window-present patch; a FOURTH file, the reverted windows-input patch "
        "reappearing, or a divergent copy of any is a supply-chain risk. Found: "
        + repr(gtk_patches)
    )
