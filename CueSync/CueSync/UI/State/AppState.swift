...
// Re-host of App/AppState.swift onto SwiftCrossUI.ObservableObject (spec CUESYNC-7 §B.3).
// The AppKit original uses Swift's `@Observable` (Observation module) and imports
// SwiftUI + Combine, neither of which exist on Windows, so it is excluded from this
// target and off-limits to edit. Every property, method name, and behavior below is
// ported verbatim from it — including the input-sanitization guards
// (`CuePoint.sanitized()`, `safeDuration`, `ensureStartAndEndPoints`) — so a hostile
// file can't produce NaN geometry here any more than it can on macOS. All parsing,
// exporting, and models are the shared, already-hardened `CueSyncCore`.
@ObservableObject
final class AppState {
    ...
    // PORT: section drag-reorder (`draggedSection`/`preDragSectionOrder` in the AppKit
    // original) is omitted — 0.8.0 has no `.draggable`/`.dropDestination` (§D.8).
    // Section order stays the fixed logical order below.
...
// MARK: - Errors
// `ParseError` is CueSyncCore.ParseError (Models/ParseError.swift), imported above.

extension JSONEncoder {
    fileprivate static var prettyEncoder: JSONEncoder {
        let e = JSONEncoder()
